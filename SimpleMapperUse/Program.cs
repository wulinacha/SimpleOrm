...
namespace SimpleMapperUse
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            //查询query方法
            var user0 = repository.Query<tb_User>("select * from tb_user where id=@id and name=@name", new { id = 60,name="bosco" }).FirstOrDefault();
  

            ////分页查找
            //var pagelist = repository.GetPageList(e => true, 1, 10);
            //foreach (var item in pagelist.Items)
            //{
            //    Console.WriteLine("分页列表：当前页" + pagelist.pageIndex+"手机号："+ item.mobile);
            //}
            //查找
